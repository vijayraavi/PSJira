...
InModuleScope JiraPS {
    ...
    Describe "Set-JiraIssueLabel" {
        ...
        Context "Behavior testing" {
            Mock Invoke-JiraMethod {
                ShowMockInfo 'Invoke-JiraMethod' 'Method', 'URI', 'Body', 'ServerName'
            }

            It "Replaces all issue labels if the Set parameter is supplied" {
                { Set-JiraIssueLabel -Issue TEST-1 -Set 'testLabel1', 'testLabel2' } | Should Not Throw
            ...
            It "Adds new labels if the Add parameter is supplied" {
                { Set-JiraIssueLabel -Issue TEST-1 -Add 'testLabel3' } | Should Not Throw
            ...
            It "Removes labels if the Remove parameter is supplied" {
                ...
                { Set-JiraIssueLabel -Issue TEST-1 -Remove 'existingLabel1' } | Should Not Throw
            ...
            It "Clears all labels if the Clear parameter is supplied" {
                { Set-JiraIssueLabel -Issue TEST-1 -Clear } | Should Not Throw
            ...
            It "Allows use of both Add and Remove parameters at the same time" {
                { Set-JiraIssueLabel -Issue TEST-1 -Add 'testLabel1' -Remove 'testLabel2' } | Should Not Throw
            }

            It "Passes the -ServerName parameter to Get-JiraIssue and Invoke-JiraMethod if specified" {
                Set-JiraIssueLabel -Issue TEST-1 -Set 'testLabel1', 'testLabel2' -PassThru -ServerName 'testServer' | Out-Null
                Assert-MockCalled -CommandName Invoke-JiraMethod -ParameterFilter {$ServerName -eq 'testServer'}
                Assert-MockCalled -CommandName Get-JiraIssue -ParameterFilter {$ServerName -eq 'testServer'} -Exactly -Times 2 -Scope it
            }
        }

        Context "Input testing" {
            It "Accepts an issue key for the -Issue parameter" {
                { Set-JiraIssueLabel -Issue TEST-1 -Set 'testLabel1' } | Should Not Throw
            ...
            It "Accepts the output of Get-JiraIssue by pipeline for the -Issue paramete" {
                { Get-JiraIssue -Key TEST-1 | Set-JiraIssueLabel -Set 'testLabel1' } | Should Not Throw
                Assert-MockCalled -CommandName Get-JiraIssue -ModuleName JiraPS -Exactly -Times 2 -Scope It
                Assert-MockCalled -CommandName Invoke-JiraMethod -ModuleName JiraPS -Exactly -Times 1 -Scope It
            }
        }
