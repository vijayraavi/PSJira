function Get-JiraIssueType {
    ...
    begin {
        $uri = "/rest/api/latest/issuetype"
        Write-Debug "[Get-JiraIssueType] Obtaining all issue types from Jira"
        $allIssueTypes = ConvertTo-JiraIssueType -InputObject (Invoke-JiraMethod -Method Get -URI $uri -ServerName $ServerName -Credential $Credential)
    }
    ...
    process {
        ...
        Write-Debug "[Get-JiraIssueType] Complete"
    }
}
